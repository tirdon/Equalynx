/// Lex and parse `input` into an `Expression` AST. This is the shared entry point
/// for the equation splitter and the token-list builder.
///
/// (GradGame's renderer/differentiator convenience helpers were dropped here — the
/// equation game never resolves derivatives or renders TeX/JS; it walks the AST
/// into draggable display tokens instead.)
func parseExpression(_ input: String) throws -> Expression {
...
private final class ExpressionParser {
    ...
    /// Caps the input so parsing — which recurses over the expression tree — cannot
    /// overflow the small Wasm stack. A flat chain of N nodes recurses N deep, and a
    /// nesting of depth D needs at least D tokens, so bounding the token count bounds
    /// every recursion depth. Without this guard a long input traps (and on Wasm can
